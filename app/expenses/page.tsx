      ...
      <div className="container">
        ...
        <div className="card" style={{ padding: 0, overflow: "hidden", marginTop: "var(--space-4)" }}>
          {loading ? (
            ...
            <ErrorState message="Could not load expenses." onRetry={load} />
          ) : !data || data.items.length === 0 ? (
            <EmptyState title="No expenses" message="Record tolls, parking, and other operational expenses." />
          ) : (
            <div style={{ overflowX: "auto" }}>
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 14 }}>
                <thead><tr><Th>Vehicle</Th><Th>Date</Th><Th>Category</Th><Th>Amount</Th><Th>Description</Th><Th></Th></tr></thead>
                <tbody>
                  {data.items.map((x) => (
                    <tr key={x.id}>
                      <Td>{x.vehicle.registrationNumber}</Td>
                      <Td>{x.date}</Td>
...
function ExpenseForm({ vehicles, record, onDone, onCancel }: { vehicles: VehicleDTO[]; record: ExpenseDTO | null; onDone: () => void; onCancel: () => void }) {
  ...
  return (
    <form onSubmit={submit} className="card">
      <h2 style={{ fontSize: 16, marginTop: 0 }}>{record ? "Edit expense" : "Add expense"}</h2>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(140px, 1fr))", gap: "var(--space-3)" }}>
        ...
        <L label="Category" err={errors.category}>
          <select value={form.category} onChange={(e) => setForm({ ...form, category: e.target.value as ExpenseCategoryValue })} style={ci(!!errors.category)}>
            <option value="">Select…</option>
